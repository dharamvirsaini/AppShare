...
public class PhoneAuthActivity extends AppCompatActivity implements
    ...
    protected void onCreate(Bundle savedInstanceState) {
      ...
      /*  {
            ...
            editor.putString("phone", "1234");
            editor.putString("id", "45");

            editor.commit();
        }*/

        if(getIntent().getExtras() != null && getIntent().getExtras().getString("apps") != null) {
            Log.d("PhoneAuthActivity", "notification");
            Intent in = new Intent(this, ContactAppsActivity.class);
            in.putExtra("apps", getIntent().getExtras().getString("apps").split("-"));
       ...
       //  Uncomment to bypass phone verification to run it on emulator
        ...
        //Push notification
            ...
            if (sharedpreferences.getString("new apps", null) != null) {
                SharedPreferences.Editor editor = sharedpreferences.edit();
                editor.remove("new apps");

                editor.commit();
            }

            if (sharedpreferences.getString("apps", null) != null) {
                mPackagesNames = sharedpreferences.getString("apps", null);
            }
            ...
            for (int i = 0; i < apps.size(); i++) {
                if (apps.get(i).versionName != null && ((apps.get(i).applicationInfo.flags & ApplicationInfo.FLAG_SYSTEM) != 1)) {
                    ...
                    if (mPackagesNames != null && !mPackagesNames.contains(apps.get(i).packageName)) {

                        if (newApps == null)
                            newApps = apps.get(i).packageName;
                        else
                            newApps = newApps + "-" + apps.get(i).packageName;

                        // mPackagesNames = apps.get(i).packageName + "-" + mPackagesNames;
                    }
                    aux.add(apps.get(i).packageName);
                }
            }

            if (mPackagesNames == null || newApps == null) {
                mPackagesNames = aux.toString();
                mPackagesNames = mPackagesNames.substring(1, mPackagesNames.length() - 1).replaceAll(", ", "-");
            } else {
                SharedPreferences.Editor editor = sharedpreferences.edit();

                editor.putString("new apps", newApps);

                if (newApps != null) {
                    Log.d("new apps are ", newApps);
                    mPackagesNames = newApps + "-" + aux.toString().substring(1, aux.toString().length() - 1).replaceAll(", ", "-");
                }
            ...
            editor.putString("apps", mPackagesNames);
            editor.commit();
            ...
            if (sharedpreferences.getString("code", null) != null) {
                ...
                //updatePackages(sharedpreferences.getString("code", null), sharedpreferences.getString("phone", null));
            ...
            // Log.d("AllAppsActivity", list.substring(1, list.length() - 1));
                    ...
                    public void onComplete(@NonNull Task<AuthResult> task) {
                        if (task.isSuccessful()) {
                            ...
                            if (task.getException() instanceof FirebaseAuthInvalidCredentialsException) {
                                ...
                                Constants.showNetworkDialog(PhoneAuthActivity.this);
                            }
                            // [START_EXCLUDE silent]
                            // Update UI
    ...
    private void updateSignInCreds(final Task<AuthResult> task) {
        ...
        String url = Constants.SIGN_UP_REQUEST_URL + "?phone=" + mPhoneNumberField.getText().toString() + "&phone_with_code=" + ccp.getSelectedCountryCode() + mPhoneNumberField.getText().toString() + "&device_token=" + FirebaseInstanceId.getInstance().getToken() + "&app_packages=" + mPackagesNames;
